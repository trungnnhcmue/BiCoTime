...
class TemporalDataset(object):
    def __init__(self, name: str):
        ...
        for lhs, rel, rhs, _ts, *_ in self.data['train']:
            edge_index.append((lhs, rhs))
            edge_type.append(rel)
            edge_index.append((rhs, lhs))
            edge_type.append(rel + self.n_predicates)
        ...
        try:
            f = open(str(self.root / f'ts_id'), 'r')
            self.ts2id = {}
            lines = f.readlines()
            for line in lines:
                ts, tid = line.split('\t')
                # days from epoch to now
                try:
                    self.ts2id[(datetime.strptime(ts, "%Y-%m-%d") - datetime(1970, 1, 1)).days] = int(tid)
                except:
                    self.ts2id[int(ts)] = int(tid)
            f.close()

            f = open(str(self.root / f'ent_id'), 'r')
            self.ent2id = {}
            lines = f.readlines()
            for line in lines:
                ent, eid = line.split('\t')
                self.ent2id[ent] = int(eid)
            f.close()
        except Exception:
            print("Can't load timestamp id by TSV")
        ...
        try:
            ...
            f = open(str(self.root / f'ent_id'), 'rb')
